...
x$dYear=as.numeric(format(x$Date,'%Y'))
x$fYear=as.numeric(format(x$Date,'%Y'))%%2
...
test$fYear=as.numeric(format(test$Date,'%Y'))%%2
# we'll set aside 2011 data as test, and train on the remaining
my.x = data.frame(x[,list(WnvPresent, dMonth, Species, Latitude, Longitude)])
x1<-my.x[x$dYear!=2011,]
x2<-my.x[x$dYear==2011,]
props<-c("dMonth","Species","Latitude","Longitude");
...
d_X <- as.matrix(d_X,rownames.force = FALSE)
d_x_valid <- as.matrix(d_x_valid,rownames.force = FALSE)
d_test<-as.matrix(d_test,rownames.force = FALSE)
testAsMatrix = data.frame(test[,list(dMonth, Species, Latitude, Longitude)])
